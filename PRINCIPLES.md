...
## 原则 6：本地 DSH 适配不参与官方对照覆盖（升级红线）

> 对照 Reasonix 官方源码升级到最新版时，**本地对 DSH 的一切适配都不允许被官方版本覆盖掉**。
> 官方升级只应带来"前端 UI 与桥方法面的更新"，本地定制是与 DSH 共存的基础，覆盖即断链。

以下类别**一律保留本地版本，不参与官方对照/替换**（升级后须从旧版本/备份恢复或重新应用）：
...
2. **前端注入脚本**：插件市场（`__DSH_PLUGIN_MARKET`，现位于 `index.html`）、错误捕获等
   本地注入，升级后按 `scripts/dsh-plugin-market-inject.js` 重新注入。
3. **桥方法适配**：`app_*.go` 中所有"DSH 语义"实现（本地持久化桥：MCP、子智能体、技能偏好、
   插件市场等）——官方实现基于 Reasonix 自身后端（skill 文件、配置服务），与 DSH 桥不同，
   升级时**不得用官方实现覆盖本地实现**，只吸收官方新增的方法面。
4. **本地持久化数据**：`~/.reasonix/` 下的用户数据（`mcp-servers.json`、
   `subagent-profiles.json`、`skill-preferences.json`、`plugins/`）是运行时数据，
   升级不涉及，也禁止升级流程触碰。

升级流程检查清单（对照官方 diff 时逐项勾选）：
- [ ] dist 中 logo/boot 品牌是否仍是本地版
- [ ] `index.html` 是否仍含插件市场注入脚本
- [ ] 桥方法：本地持久化实现（MCP/子智能体/技能偏好/插件市场）未被官方实现替换
- [ ] 本地独有桥方法（`DshStd*`、`MarketPage`、`Terminal*` 等 23 个）未被删除
- [ ] `~/.reasonix/` 用户数据完整
