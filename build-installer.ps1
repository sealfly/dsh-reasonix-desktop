...
# Built-in skills must ship: frontend/dist gets regenerated by upstream upgrades, and skills/
# is just as easy to forget. Missing it means the packaged app seeds no built-in skill at all
# (the generator skill silently disappears on a fresh machine).
if (-not (Test-Path (Join-Path $root "skills"))) {
  Write-Error "skills/ not found: built-in skills would not ship with the installer"
  exit 1
}
