...
**一句话**：桥不设卡，但 UI 有边界——DSH 有多少能力是 DSH 的事，UI 展示多少是 Reasonix 官方设计的事；我们作为外壳，保证桥的透传是通的，但不越界替前端发明它没有的界面。
...
| 外壳 | 位置 | 干什么 |
|---|---|---|
| Electron 版 | `src/preload.js` + `src/main.js` | 注入桥 + 窗口/系统能力 |
| Wails 版 | Go 后端（`*.go`） | 绑定方法 + 系统能力 |

**为什么要零改动**：

1. **可升级**：官方 Reasonix 发新版时，只需替换 dist，外壳不用跟着改；若改过 dist，升级就得重新手动合并改动。
2. **不破坏官方行为**：前端是官方精心构建的，改动极易引入 bug——我们之前踩的坑（叠影、崩溃、锁浅色）本质上都是"在错误的地方打补丁"造成的。
3. **职责分离**：前端管 UI，外壳管桥接，边界清晰，各司其职。
...
## 实时事件流通路（通路 A）——已实现（2026-08）

Reasonix 前端有两条内容通路：A. `agent:event` 事件流；B. `topic:activation` → hydrate → `HistorySliceForTab`。两者都已完成：
