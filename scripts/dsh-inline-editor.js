...
// 载入编辑器；Ctrl+S 或「保存」→ WriteFileForTab 写回后恢复预览。
//
// 编辑器引擎（v4）：
//   - 优先 Monaco Editor（VS Code 内核，assets/monaco/vs 离线内置）——语法高亮/行号/
//     括号自动配对/查找替换/多光标/自动补全(Tab 接受)；
//   - Monaco 加载失败或创建异常 → 自动回退 textarea 轻量版（自动补全弹层仍可用）——
//     失败留痕兜底，不阻塞编辑。
//
...
(function () {
  ...
  var editorInst = null;    // Monaco editor 实例（textarea 模式为 null）
  ...
  // "改动"标签页预览区专属类名（WorkspacePanel 打包产物里这三个只在
  // 视图状态 He==="changed" 且选中文件时渲染：当前改动 diff / 提交历史）。
  // 这些视图复用同一个 .workspace-preview__body，但显示的不是文件内容——
  // 在那里注入编辑按钮会指向文件树里选中的文件（而不是 diff 里的文件），必须屏蔽。
  // 注意：.workspace-change-scope / .workspace-change-row 是通用渲染函数
  // （renderChangeScope），文件视图也可能出现，不能作为判据。
  var CHANGE_VIEW_SELECTORS = [
    ".workspace-current-change",
    ".workspace-git-history",
    ".workspace-commit-history",
  ];
  // 另一条入口：切到"改动"标签后文件树被卸载 → 没有活动行 → 按钮必须跟着移除，
  // 否则它会残留在共用的 .workspace-preview__window-actions 上（见 editablePreview 的 currentRel 检查）。

  // inChangeView 当前右侧栏是否处于"改动"视图。
  function inChangeView() {
    for (var i = 0; i < CHANGE_VIEW_SELECTORS.length; i++) {
      try { if (document.querySelector(CHANGE_VIEW_SELECTORS[i])) return true; } catch (e) {}
    }
    return false;
  }

  // editablePreview 当前预览区是否真的是"可编辑的文件内容预览"：
  // 有活动文件 + 有预览容器 + 不在改动视图。
  ...
  // hideEditBtn 移除编辑按钮；若正在编辑则先退出（切到改动视图时调用）。
  ...
  // 仅文件内容预览注入；改动/diff 与提交历史视图一律移除（见 editablePreview）。
  // 按钮文案按当前是否在编辑态切换（"✏️ 编辑"/"退出编辑"）。
  function ensureEditBtn() {
    if (!editablePreview()) { hideEditBtn(); return; }
  ...
  // 3) 尝试 Monaco（加载 + 创建都成功才用）；任一失败 → textarea 轻量版回退；
  // 4) Ctrl+S 保存、Esc 退出。
  function enterEdit() {
    if (overlay) return;
    if (inChangeView()) { toast("改动视图不支持就地编辑，请切到「文件」标签"); return; }
    var rel = currentRel();
    ...
    resolveTab(function (tid) {
      ...
      app.ReadFileForTab(tid, rel).then(function (pv) {
        ...
        // Monaco 优先；加载/创建失败回退 textarea（ensureMonaco 内部失败即回退）
        openMonacoEditor(pv, body);
      }).catch(function (err) { toast("读取失败: " + String(err && err.message || err)); });
    });
  }

  // ===== Monaco 引擎（VS Code 内核，离线内置 assets/monaco/vs） =====
  var monacoPending = null; // 加载队列（并发请求复用一次加载）

  // ensureMonaco 用 AMD loader 加载 monaco（一次性；失败/缺失 → 回退调用方）。
  function ensureMonaco(cb) {
    if (window.monaco) { cb(true); return; }
    if (monacoPending) { monacoPending.push(cb); return; }
    monacoPending = [cb];
    var settle = function (ok) {
      var q = monacoPending; monacoPending = null;
      (q || []).forEach(function (f) { try { f(ok); } catch (e) {} });
    };
    var vsPath = "./assets/monaco/vs";
    var ls = document.createElement("script");
    ls.src = vsPath + "/loader.js";
    ls.onload = function () {
      try {
        window.require.config({ paths: { vs: vsPath } });
        window.require(["vs/editor/editor.main"], function () {
          settle(true);
        }, function (err) { console.warn("[dsh-ie] monaco editor.main load fail", err); settle(false); });
      } catch (e) { console.warn("[dsh-ie] monaco init fail", e); settle(false); }
    };
    ls.onerror = function () { console.warn("[dsh-ie] monaco loader missing (offline copy absent)"); settle(false); };
    document.head.appendChild(ls);
  }

  // monacoLangId 扩展名 → Monaco languageId（unknown → plaintext，由调用方兜底高亮缺失）。
  function monacoLangId(rel) {
    if (!rel) return "plaintext";
    var i = rel.lastIndexOf(".");
    if (i < 0) return "plaintext";
    var ext = rel.slice(i).toLowerCase();
    var map = {
      ...
      ".java": "java", ".json": "json", ".yaml": "yaml", ".yml": "yaml", ".sh": "shell",
      ".bash": "shell", ".ps1": "powershell", ".sql": "sql", ".xml": "xml",
      ".html": "html", ".htm": "html", ".css": "css", ".scss": "scss", ".less": "less",
      ".md": "markdown", ".markdown": "markdown", ".txt": "plaintext", ".log": "plaintext",
      ".toml": "ini", ".ini": "ini", ".conf": "ini", ".dockerfile": "dockerfile",
      ".vue": "html", ".rb": "ruby", ".php": "php", ".kt": "kotlin", ".swift": "swift",
      ".cs": "csharp", ".dart": "dart", ".lua": "lua", ".r": "r", ".pl": "perl"
    };
    return map[ext] || "plaintext";
  }

  // registerKeywordsProvider 给某 languageId 注册补全源（语言关键字 + 文件内标识符）。
  // 与 textarea 版同一份 KEYWORDS 表；按 ext 查关键字。
  var providerDone = {}; // 防重复注册
  function registerKeywordsProvider(langId, ext) {
    if (providerDone[langId]) return;
    providerDone[langId] = true;
    try {
      monaco.languages.registerCompletionItemProvider(langId, {
        ...
        provideCompletionItems: function (model, position) {
          ...
          var kws = (KEYWORDS[ext] || []).map(function (k) {
            return { label: k, kind: monaco.languages.CompletionItemKind.Keyword, insertText: k, range: range };
          });
          var ids = {}, m, re = /[A-Za-z_$][A-Za-z0-9_$]*/g, txt = model.getValue();
          while ((m = re.exec(txt))) { if (m[0].length >= 3) ids[m[0]] = true; }
          var identItems = Object.keys(ids).slice(0, 200).map(function (id) {
            return { label: id, kind: monaco.languages.CompletionItemKind.Variable, insertText: id, range: range };
          });
          return { suggestions: kws.concat(identItems) };
        }
      });
    } catch (e) { console.warn("[dsh-ie] provider register fail", e); }
  }

  // buildOverlayShell 建覆盖层骨架（bar + 保存按钮 + 空 host），两引擎共用。
  // 返回 {ovl, save, host}；调用方自行填充 host 内容。
  function buildOverlayShell(pv, hintText) {
    var body = previewBody();
    ...
    nm.className = "nm";
    nm.textContent = relPath;
    var sp = document.createElement("span");
    sp.className = "sp";
    var ht = document.createElement("span");
    ht.className = "ht";
    ht.textContent = pv && pv.truncated ? "内容超限已截断，保存会覆盖" : hintText;
    ...
    host.className = NS + "-mh";
    ovl.appendChild(bar);
    ovl.appendChild(host);
    body.appendChild(ovl);
    overlay = ovl;
    var b = document.querySelector("." + NS + "-btn");
    if (b) b.textContent = "退出编辑";
    return { ovl: ovl, save: save, host: host };
  }

  // openMonacoEditor Monaco 版编辑：加载成功且创建成功才用；否则回退 textarea。
  function openMonacoEditor(pv, body) {
    ensureMonaco(function (ok) {
      if (!ok) { openTaEditor(pv, body); return; }
      try {
        var sh = buildOverlayShell(pv, "Ctrl+S 保存 · Esc 退出 · Tab 补全");
        var langId = monacoLangId(relPath);
        var ext = langId === "plaintext" ? "" : relPath.slice(relPath.lastIndexOf("."));
        editorInst = monaco.editor.create(sh.host, {
          value: (pv && pv.body) || "",
          language: langId,
          theme: "vs-dark",
        ...
        if (ext && KEYWORDS[ext]) registerKeywordsProvider(langId, ext);
        editorInst.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyS, doSave);
        editorInst.addCommand(monaco.KeyCode.Escape, exitEdit);
        editorInst.focus();
      } catch (e) {
        console.warn("[dsh-ie] monaco create fail, fallback textarea:", e);
        if (editorInst) { try { editorInst.dispose(); } catch (e2) {} editorInst = null; }
        if (overlay && overlay.parentNode) overlay.parentNode.removeChild(overlay);
        overlay = null;
        var bb = document.querySelector("." + NS + "-btn");
        if (bb) bb.textContent = "✏️ 编辑";
        openTaEditor(pv, body);
      }
    });
  }

  // openTaEditor textarea 轻量版（Monaco 不可用时的回退；自动补全弹层逻辑完整保留）。
  function openTaEditor(pv, body) {
    var sh = buildOverlayShell(pv, "Tab 缩进 · Ctrl+S 保存 · Esc 退出");
    var ta = document.createElement("textarea");
    ta.className = NS + "-ta";
    ta.spellcheck = false;
    ta.value = (pv && pv.body) || "";
    ta.__lang = langOf(relPath);
    ...
    sh.host.appendChild(ta);
    ta.focus();
  }

  // currentTA 取当前 overlay 里的 textarea（仅 textarea 模式；Monaco 模式返回 null）。
  function currentTA() {
    return overlay ? overlay.querySelector("." + NS + "-ta") : null;
  }

  // getEditorText 取当前编辑器内容（Monaco → getValue；textarea → value）。
  function getEditorText() {
    if (editorInst) { try { return editorInst.getValue(); } catch (e) { return ""; } }
    var ta = currentTA();
    return ta ? ta.value : "";
  }

  // doSave 把当前编辑器内容经 WriteFileForTab 写回文件：
  ...
  function doSave() {
    ...
    app.WriteFileForTab(tabId, relPath, text).then(function (r) {
      if (r && r.ok) {
        toast("✓ 已保存 " + relPath);
        exitEdit();
  ...
  // 退出编辑：释放 Monaco 实例 + 仅移除自己的 overlay（React 节点零触碰）
  function exitEdit() {
    if (editorInst) {
      try { editorInst.dispose(); } catch (e) {}
      editorInst = null;
    }
    hideComplete();
    if (overlay && overlay.parentNode) overlay.parentNode.removeChild(overlay);
  ...
  function scan() {
    ...
    // overlay 节点被外部移除(React 重建 body)而状态残留 → 复位状态与按钮文案
    if (overlay && !overlay.isConnected) {
      if (editorInst) { try { editorInst.dispose(); } catch (e) {} editorInst = null; }
      hideComplete();
      overlay = null; relPath = "";
      var bb = document.querySelector("." + NS + "-btn");
      if (bb) bb.textContent = "✏️ 编辑";
    }
  }
