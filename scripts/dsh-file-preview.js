...
// 集成在右侧文件栏：运行时探测右侧面板 → 面板头部注入"📁 文件"入口 →
// 文件浏览（ListWorkspaceFiles）+ 预览弹层（PreviewFile）。
// 对话区内点击本地文件路径/链接 → 自动预览。
// 自包含（无前端锚点依赖）；桥调用走 wails 绑定，透传原则。
(function () {
  ...
  var css = [
    "." + NS + "__rt-btn{display:inline-flex;align-items:center;gap:5px;margin:6px 8px;padding:5px 10px;border-radius:7px;border:1px solid rgba(128,128,128,.35);background:rgba(28,29,33,.9);color:#f4f4f3;font-size:12px;font-weight:500;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.3);transition:background .15s;white-space:nowrap}",
    "." + NS + "__rt-btn:hover{background:#3b3f46}",
    "." + NS + "__panel{position:fixed;right:16px;bottom:70px;z-index:99991;width:360px;max-height:62vh;display:flex;flex-direction:column;background:rgba(24,25,29,.97);color:#f4f4f3;border:1px solid rgba(128,128,128,.4);border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.6);font:13px Inter,system-ui,sans-serif;overflow:hidden}",
  ...
  // ===== 右侧文件栏集成 =====
  // 运行时探测右侧面板（Reasonix 混淆代码无稳定类名，用几何特征定位：
  // 贴右 80px 内、高度 >35% 视口的最大容器），在面板头部注入文件入口。
  function findRightPanel() {
    var best = null, bestScore = 0;
    var all = document.querySelectorAll("aside, div, section, nav, main");
    for (var i = 0; i < all.length; i++) {
      var el = all[i];
      var r = el.getBoundingClientRect();
      if (r.width < 80 || r.width > 900) continue;
      if (r.height < window.innerHeight * 0.35) continue;
      var gap = window.innerWidth - r.right;
      if (gap >= -5 && gap <= 80) {
        var score = r.height + (r.width > 200 ? 500 : 0);
        if (score > bestScore) { bestScore = score; best = el; }
      }
    }
    return best;
  }
  function mountFileButton(panel) {
    if (panel.querySelector("." + NS + "__rt-btn")) return;
    var btn = document.createElement("button");
    btn.className = NS + "__rt-btn";
    btn.textContent = "📁 文件";
    btn.title = "浏览并预览工作区文件（md/docx/xlsx/pptx）";
    btn.addEventListener("click", function (e) {
      e.stopPropagation();
      openBrowser(currentCwd());
    });
  ...
  function initRightPanel() {
    ...
    var timer = setInterval(function () {
      tries++;
      var panel = findRightPanel();
      if (panel) {
        clearInterval(timer);
        mountFileButton(panel);
      } else if (tries > 60) {
        clearInterval(timer); // 12s 上限，找不到就放弃（不干扰 UI）
      }
    }, 200);
  }
  // 前端面板重建时重新注入（MutationObserver 观察右侧面板区域）
  var mo = new MutationObserver(function () {
    var panel = findRightPanel();
    if (panel) mountFileButton(panel);
  });
  var moTries = 0;
