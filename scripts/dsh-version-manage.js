/* __DSH_VERSION_MANAGE v1 (DSH 核心版本管理 - 设置页区块, 本地注入) */
// dsh-version-manage.js — 在设置面板追加「DSH 核心版本」管理区块：
// 当前版本 / 最新版本 / npm 版本列表选择 / GitHub Releases（含 win-x64 安装包直链下载）。
...
(function () {
  ...
  function inject() {
    if (injected) return;
    // 锚点：设置面板内容区（settings-center__content），挂到内容底部
    var host = document.querySelector(".settings-center__content, .settings-center");
    if (!host) return;
    injected = true;
